...
# Versão com salvamento de modelos
...
# Função para salvar modelo completo
def salvar_modelo_completo(model, optimizer, errors, epochs, mse, tempo, train_num):
    """
    Salva o modelo treinado com todas as informações relevantes
    """
    modelo_info = {
        ...
        'model_class': MLP  # Para reconstrução do modelo
    }
    
    # Salvar usando torch (recomendado para modelos PyTorch)
    torch.save(modelo_info, f'Modelos/modelo_T{train_num}.pth')
    
    # Salvar também em pickle para compatibilidade
    with open(f'Modelos/modelo_T{train_num}.pkl', 'wb') as f:
        pickle.dump(modelo_info, f)
    
    print(f"    ✓ Modelo T{train_num} salvo em: Modelos/modelo_T{train_num}.pth")

# Função para carregar modelo salvo
def carregar_modelo(caminho_modelo):
    """
    Carrega um modelo salvo e reconstrói a rede neural
    """
    if caminho_modelo.endswith('.pth'):
        modelo_info = torch.load(caminho_modelo, map_location='cpu')
    else:
    ...
    model = MLP()
    model.load_state_dict(modelo_info['model_state_dict'])
    
    return model, modelo_info
...
def treinar_rede(X_train, y_train, learning_rate=0.1, max_epochs=3000000, target_error=1e-6, train_num=1):
    ...
    with torch.no_grad():
        outputs = model(X_train)
        mse_final = criterion(outputs, y_train).item()
    
    # Salvar modelo completo
    salvar_modelo_completo(model, optimizer, errors, epoch, mse_final, tempo_treinamento, train_num)
...
print("- Função de ativação: Sigmoide (camada oculta) + Linear (saída)")
print("- Algoritmo: Backpropagation (SGD com momentum 0.9)")
print("- Modelos serão salvos em: Modelos/")
print("-"*70)
...
print(f"✓ Todos os modelos salvos na pasta: Modelos/")
print("="*70)
...
for i, resultado in enumerate(resultados_treinamento):
    ...
    ws1[f'D{i+2}'] = f"modelo_T{resultado['treinamento']}.pth"
...
# Criar arquivo de resumo dos modelos
print("\nCriando arquivo de resumo dos modelos...")
with open('Modelos/resumo_modelos.txt', 'w', encoding='utf-8') as f:
    f.write("RESUMO DOS MODELOS TREINADOS - PROJETO RNA ETAPA 01\n")
    f.write("="*60 + "\n\n")
    
    f.write("CONFIGURAÇÕES DE TREINAMENTO:\n")
    f.write("- Arquitetura: 3 → 10 → 1\n")
    f.write("- Função ativação oculta: Sigmoide\n")
    f.write("- Função ativação saída: Linear\n")
    f.write("- Taxa de aprendizado: 0.1\n")
    f.write("- Momentum: 0.9\n")
    f.write("- Precisão alvo: 1e-6\n")
    f.write("- Máximo de épocas: 3.000.000\n\n")
    
    f.write("RESULTADOS POR MODELO:\n")
    f.write("-" * 60 + "\n")
    
    for i, resultado in enumerate(resultados_treinamento):
        erro_rel = erros_relativos[i]
        f.write(f"MODELO T{resultado['treinamento']}:\n")
        f.write(f"  Arquivo: modelo_T{resultado['treinamento']}.pth\n")
        f.write(f"  Épocas: {resultado['epochs']:,}\n")
        f.write(f"  MSE final: {resultado['mse']:.2e}\n")
        f.write(f"  Tempo treinamento: {resultado['tempo']:.2f}s\n")
        f.write(f"  Erro relativo médio: {erro_rel['erro_medio']:.2f}%\n")
        f.write(f"  Variância: {erro_rel['variancia']:.2f}%\n\n")
    
    f.write("RECOMENDAÇÃO:\n")
    f.write("-" * 60 + "\n")
    f.write(f"Melhor modelo: T{melhor_treinamento['treinamento']}\n")
    f.write(f"Erro relativo médio: {melhor_treinamento['erro_medio']:.2f}%\n")
    f.write(f"Arquivo: modelo_T{melhor_treinamento['treinamento']}.pth\n\n")
    
    f.write("INSTRUÇÕES DE USO:\n")
    f.write("-" * 60 + "\n")
    f.write("Para carregar um modelo:\n")
    f.write("```python\n")
    f.write("model, info = carregar_modelo('Modelos/modelo_T1.pth')\n")
    f.write("model.eval()\n")
    f.write("with torch.no_grad():\n")
    f.write("    output = model(input_tensor)\n")
    f.write("```\n")

print("✓ Resumo dos modelos salvo em: Modelos/resumo_modelos.txt")
...
print(f"Arquivo do modelo: modelo_T{melhor_treinamento['treinamento']}.pth")
...
print("\nMODELOS:")
print("✓ Modelos/modelo_T1.pth")
print("✓ Modelos/modelo_T2.pth")
print("✓ Modelos/modelo_T3.pth")
print("✓ Modelos/modelo_T4.pth")
print("✓ Modelos/modelo_T5.pth")
print("✓ Modelos/resumo_modelos.txt")
print("="*70)
print("PROJETO ETAPA 01 CONCLUÍDO COM SUCESSO!")

# Exemplo de como carregar e usar um modelo salvo
print(f"\n{'='*70}")
print("EXEMPLO DE USO DOS MODELOS SALVOS:")
print("="*70)
print("# Carregar o melhor modelo")
print(f"model, info = carregar_modelo('Modelos/modelo_T{melhor_treinamento['treinamento']}.pth')")
print("model.eval()")
print("")
print("# Fazer predição com novos dados")
print("# novo_input = torch.FloatTensor([[x1, x2, x3]])")
print("# with torch.no_grad():")
print("#     predicao = model(novo_input)")
print("#     print(f'Energia absorvida estimada: {predicao.item():.4f}')")
print("="*70)